...
internals.payloadError = (request, h) => {
...
internals.failAction = (request, h, error) => {
  ...
  request.app.validationErrors = request.app.validationErrors.concat(
    error.details || error
  );
...
internals.SignUp = (request, h) => {
  ...
  if (request.app.payloadError) {
    errors = internals.buildUsefulErrorObject([
      { path: ["global"], type: "payload" },
    ]);
  }
